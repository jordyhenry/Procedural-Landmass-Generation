Shader "Custom/Terrain" {
	Properties
	{
		...
		testTextureScale("Texture Scale", Float) = 1
	}
	SubShader{
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			for (int i = 0; i < layerCount; i++)
			{
				...
				float3 textureColor = triplanar(IN.worldPos, baseTexturesScales[i], blendAxis, i) * (1 - baseColorsStrength[i]);
